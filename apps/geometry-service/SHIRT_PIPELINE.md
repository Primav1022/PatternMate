...
| 1 | `body` | 领或门襟相对底款变了 → **一次整换**前后衣身（含侧片、领附件、育克、门襟） |
...
| 操作 | 执行 |
|---|---|
| 领型 + 前门襟 | 一起整换 `BODY_SWAP_ROLES`（前后片 + 侧片 + 领 + 门襟 + 育克） |
| 廓形 | 不换片。从供体取**一条左、一条右**侧缝（半片只取外侧，不要前中），前后片和侧片共用；cut 和 sew 都改。臂根接点锁住，下摆 y 锁住，下摆 x 可外放/收进 |
...
| 放码后检测 | 分片后幅（后片宽远小于后片+侧片）给出提示；胸围系数≥1 而衣身胸宽变窄则报警 |
