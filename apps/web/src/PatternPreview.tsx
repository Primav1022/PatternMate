...
function DesignOverview({ recipe, patternContext, styleKey, ready, generationRevision = 0, seedPreviewUrl, onGenerated }: { recipe: CompositionRecipe; patternContext: Record<string, any>; styleKey: string; ready: boolean; generationRevision?: number; seedPreviewUrl?: string; onGenerated?: (url: string, input: Record<string, any>, revision: number) => void }) {
  const { t } = useLanguage();
  const [job, setJob] = useState<any>(null);
  const [imageUrl, setImageUrl] = useState(seedPreviewUrl || '');
  const [selectedUrl, setSelectedUrl] = useState(seedPreviewUrl || '');
  ...
  const buildInput = (instruction?: string) => ({
  ...
  const loadPrompt = async (instruction = userInstruction) => {
    setPromptLoading(true);
    try {
      const base = import.meta.env.VITE_AI_BASE_URL || '/ai';
      const response = await fetch(`${base}/design-preview/prompt`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(buildInput(instruction)) });
  ...
  const generate = async (withPrompt = false) => {
    ...
    const input = buildInput(withPrompt ? userInstruction : undefined);
    lastInputRef.current = input;
    setError(''); setJob({ status: 'queued', progress: 0 }); setPromptOpen(false);
    try {
      const base = import.meta.env.VITE_AI_BASE_URL || '/ai';
      ...
      if (current.status !== 'succeeded' || !current.result_urls?.[0]) throw new Error(String(current.error || t('2D设计生成失败，请重试。', '2D design generation failed. Please retry.')));
      if (current.style_key !== styleKey) return;
      ...
      if (current.prompt) setPromptPreview(String(current.prompt));
  ...
  const confirmPreview = () => {
    if (!imageUrl || !imageReady || job) return;
    const input = lastInputRef.current.style_key ? lastInputRef.current : buildInput();
    setSelectedUrl(imageUrl);
    onGenerated?.(imageUrl, input, generationRevision);
  };
  useEffect(() => {
  ...
  }, [generationRevision, styleKey]);
  useEffect(() => {
    if (!ready || !recipe.base_case_id || autoGenerated.current) return;
    autoGenerated.current = true; void generate(false);
  }, [recipe.base_case_id, ready, generationRevision, styleKey]);
  const waiting = Boolean(job) || Boolean(imageUrl && !imageReady) || (!imageUrl && !error);
  return <div className="design-ai-preview">
    {imageUrl && <button type="button" className={`design-ai-image${selectedUrl === imageUrl ? ' selected' : ''}`} aria-pressed={selectedUrl === imageUrl} onClick={confirmPreview} disabled={!imageReady || Boolean(job)}><img src={imageUrl} alt={t('2D设计预览', '2D design preview')} className={imageReady ? 'ready' : ''} onLoad={() => setImageReady(true)} />{selectedUrl === imageUrl && imageReady && <span>{t('已选定，可进入下一步', 'Selected — ready for the next step')}</span>}</button>}
    ...
    {promptOpen && <div className="design-ai-prompt">
      <label><span>{t('内置提示词（只读）', 'Built-in prompt (read only)')}</span><textarea value={promptPreview} readOnly rows={10} placeholder={promptLoading ? t('正在加载提示词…', 'Loading prompt…') : t('生成提示词', 'Generation prompt')} /></label>
      <label><span>{t('补充要求', 'Additional instruction')}</span><textarea value={userInstruction} onChange={(event) => setUserInstruction(event.target.value)} rows={3} placeholder={t('例如：背景为简洁的室内工作室', 'For example: use a minimal indoor studio background')} /></label>
      <div className="design-ai-prompt-actions">
        <button className="secondary" disabled={promptLoading} onClick={() => { setUserInstruction(''); void loadPrompt(''); }}>{t('清空补充', 'Clear addition')}</button>
        <button className="primary" disabled={!ready || promptLoading} onClick={() => { void generate(true); }}>{t('使用补充要求生成', 'Generate with addition')}</button>
      </div>
    </div>}
  </div>;
}
...
export function PatternPreview({ recipe, baseCoverUrl, styleKey, generationRevision = 0, seedPreviewUrl, styleVersions = [], activeVersionId = '', onRestoreVersion, onGeneratedPreview, onReplaceSelection, onRevertInvalid, onExport, onValidationChange, onCompositionChange }: {
  ...
  onRevertInvalid?: () => void;
  onExport: () => void;
  onValidationChange?: (ready: boolean) => void;
  onCompositionChange?: (summary: { recipe_hash: string; pieces: Piece[]; sizing_profile: Record<string, string | number>; sources: ComposeResult['sources'] }) => void;
}) {
  const { t } = useLanguage();
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (data.status === 'valid') {
          ...
          compositionCallback.current?.({ recipe_hash: data.recipe_hash, pieces: data.pieces, sizing_profile: data.sizing_profile, sources: data.sources || {} });
        }
      } catch (requestError) {
  ...
  const designPatternContext = { recipe_hash: visibleResult?.recipe_hash || '', pieces: (visibleResult?.pieces || []).map((piece) => ({ role: piece.role, width_mm: piece.width_mm, height_mm: piece.height_mm })), sizing_profile: visibleResult?.sizing_profile || {}, sources: visibleResult?.sources || {} };
  ...
  return <div className="pattern-workbench" style={{ '--fabric-color': recipe.fabric_color || '#ffffff' } as React.CSSProperties}>
    <div className={`pattern-stage ${viewMode === 'design' ? 'design-mode' : ''}`}>
      ...
      <div className="design-preview-stage" style={{ display: viewMode === 'design' ? undefined : 'none' }}>
        <DesignOverview recipe={recipe} patternContext={designPatternContext} styleKey={styleKey} ready={designReady} generationRevision={generationRevision} seedPreviewUrl={seedPreviewUrl} onGenerated={onGeneratedPreview} />
      ...
      {result?.status === 'invalid' && hasComponentAdjustments && <div className="compose-error">
        ...
        <div><button disabled={!lastValid} onClick={() => onRevertInvalid?.()}>{t('返回上一有效组合', 'Return to last valid combination')}</button>{replacement && <button className="primary" onClick={() => { setAttemptedReplacement(`${replacement[0]}:${replacement[1][0].option_id}`); onReplaceSelection(replacement[0], replacement[1][0].option_id); }}>{t('使用已验证的', 'Use validated')} “{replacement[1][0].label}”</button>}</div>
      </div>}
