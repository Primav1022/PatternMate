...
type DesignPreviewState = { url: string; input: Record<string, any>; revision: number; styleKey: string };
type ValidStyleSnapshot = {
  styleKey: string;
  ...
  revision: number;
  composition: any;
  designPreview: DesignPreviewState | null;
};
type ReferenceItem = {
  id: string;
  label: string;
...
function stableStyleKey(value: Record<string, any>): string {
  ...
  return `style-${(first >>> 0).toString(16).padStart(8, '0')}${(second >>> 0).toString(16).padStart(8, '0')}`;
}
...
const fallbackReferences: ReferenceItem[] = MOCK_REFERENCE_IDS.map((id, index) => {
  ...
  return {
    ...
    supported: true,
    coverUrl: `/reference-images/v1/${id}/cover.jpg`,
    semantics: {
...
function HomePage({ language, setLanguage, onStart }: { language: 'zh' | 'en'; setLanguage: (value: 'zh' | 'en') => void; onStart: () => void }) {
  ...
  const gallery = [1, 4, 3, 7, 6, 9, 5, 8].map((index) => `/home-gallery/garment-${String(index).padStart(2, '0')}.png`);
...
function App() {
  ...
  const [finalDesignPreview, setFinalDesignPreview] = useState<DesignPreviewState | null>(null);
  const [lastValidStyle, setLastValidStyle] = useState<ValidStyleSnapshot | null>(null);
  const [styleVersions, setStyleVersions] = useState<Array<{ id: string; label: string; revision: number; styleKey: string; selections: Record<string, string | null>; materialId: string; fabricColor: string; processId: string; recipeHash: string; designUrl: string; designInput: Record<string, any>; pieceCount: number }>>([]);
  ...
  const [printAssets, setPrintAssets] = useState<PrintAsset[]>([
    { id: 'builtin-01', name: '内置图案 01', src: '/print-library/print-test-1/source.png' },
    { id: 'builtin-02', name: '内置图案 02', src: '/print-library/print-test-2/source.png' },
    { id: 'builtin-03', name: '内置图案 03', src: '/print-library/print-test-3/source.png' },
  ]);
  ...
  const submittedStyleKey = useMemo(() => stableStyleKey({
    ...
    fabric_color: recipe.fabric_color,
    process_id: recipe.process_id,
  }), [recipe.family, recipe.sex, recipe.base_case_id, recipe.measurements_cm, recipe.selections, recipe.material_id, recipe.fabric_color, recipe.process_id]);
  ...
  const chooseReference = (id: string) => {
    ...
    setFinalDesignPreview(null); setLastValidStyle(null); setDesignPreviewRevision(0); setStyleVersions([]);
  };
  ...
  const stylingReady = !hasDraftPatternChanges && compositionReady && Boolean(finalDesignPreview?.url && finalDesignPreview.styleKey === submittedStyleKey);
  ...
  const applyValidatedReplacement = (group: string, optionId: string) => {
    const next = { ...selections, [group]: optionId };
    setSelections(next); setSubmittedSelections(next);
    setSubmittedMaterialId(materialId); setSubmittedFabricColor(fabricColor); setSubmittedProcessId(processId);
    setFinalDesignPreview(null);
    setDesignPreviewRevision((value) => value + 1);
    setCompositionReady(false); setStylingConfirmed(false);
  };
  const restoreLastValidStyle = () => {
    if (!lastValidStyle) return;
    setSelections({ ...lastValidStyle.selections }); setSubmittedSelections({ ...lastValidStyle.selections });
    setMaterialId(lastValidStyle.materialId); setSubmittedMaterialId(lastValidStyle.materialId);
    setFabricColor(lastValidStyle.fabricColor); setSubmittedFabricColor(lastValidStyle.fabricColor);
    setProcessId(lastValidStyle.processId); setSubmittedProcessId(lastValidStyle.processId);
    setDesignPreviewRevision(lastValidStyle.revision);
    setCompositionSummary(lastValidStyle.composition);
    setFinalDesignPreview(lastValidStyle.designPreview);
    setCompositionReady(true); setStylingConfirmed(false);
  };
  const restoreStyleVersion = (version: typeof styleVersions[number]) => {
    ...
    setFinalDesignPreview({ url: version.designUrl, input: version.designInput, revision: version.revision, styleKey: version.styleKey });
    setCompositionReady(false);
    setStylingConfirmed(false);
  };
  const acceptComposition = (summary: any) => {
    setCompositionSummary(summary);
    setLastValidStyle((current) => ({
      styleKey: submittedStyleKey,
      ...
      composition: summary,
      designPreview: finalDesignPreview?.styleKey === submittedStyleKey
        ? finalDesignPreview
        : current?.styleKey === submittedStyleKey ? current.designPreview : null,
    }));
  };
  const acceptGeneratedPreview = (url: string, input: Record<string, any>, revision: number) => {
    const styleKey = String(input.style_key || '');
    if (!styleKey || styleKey !== submittedStyleKey) return;
    const preview = { url, input, revision, styleKey };
    if (lastValidStyle?.styleKey === styleKey) setCompositionReady(true);
    setFinalDesignPreview(preview);
    setLastValidStyle((current) => current?.styleKey === styleKey ? { ...current, revision, designPreview: preview } : current);
  };
  useEffect(() => {
  ...
  }, [stylingReady, compositionSummary, finalDesignPreview, designPreviewRevision, submittedStyleKey, submittedSelections, submittedMaterialId, submittedFabricColor, submittedProcessId]);
  ...
  return <div className="app-shell" style={{ '--theme-primary': fixedTheme.primary, '--theme-soft': fixedTheme.soft, '--theme-border': fixedTheme.border } as React.CSSProperties}><header className="topbar"><button type="button" className="brand-lockup" onClick={() => setShowHome(true)} aria-label="PatternMate"><img className="brand-mark" src="/brand/logo.png" alt="" /><img className="brand-wordmark" src="/brand/patternmate-wordmark.svg" alt="PatternMate" /></button><div className="project-title-view"><span title={projectName}>{isUntitledProjectName(projectName) ? t('新建文件', 'New file') : projectName}</span><button aria-label={t('编辑项目名称', 'Edit project name')} title={t('编辑项目名称', 'Edit project name')} onClick={() => { setProjectNameDraft(projectName); setProjectNameEditing(true); }}>✎</button></div><nav className="steps">{steps.map((item, index) => <button key={item.id} disabled={!canAccess(item.id)} className={item.id === step ? 'step active' : 'step'} onClick={() => canAccess(item.id) && setStep(item.id)}><span aria-hidden="true">{index + 1}</span>{item.label}</button>)}</nav><label className="language-picker" aria-label={t('语言', 'Language')}><select aria-label={t('语言', 'Language')} value={language} onChange={(event) => setLanguage(event.target.value as 'zh' | 'en')}><option value="zh">中文</option><option value="en">English</option></select></label>{stylingConfirmed && <button className="topbar-export" disabled={exporting} onClick={exportAll}>{exporting ? t('生成中…', 'Generating…') : t('导出', 'Export')}</button>}</header>
    ...
    <main className="workspace resizable-workspace" style={{ gridTemplateColumns: `${sidebarWidth}px minmax(0,1fr)` }}><aside className="sidebar"><div className="side-heading"><img className="side-heading-mark" src={`/icon/Group ${{ measure: 278, design: 279, styling: 280, print: 282 }[step]}.svg`} alt="" aria-hidden="true" />{current.label}</div>
      ...
      {step === 'styling' && <PatternPreview recipe={recipe} baseCoverUrl={selectedReferenceInfo.coverUrl} styleKey={submittedStyleKey} generationRevision={designPreviewRevision} seedPreviewUrl={finalDesignPreview?.revision === designPreviewRevision && finalDesignPreview.styleKey === submittedStyleKey ? finalDesignPreview.url : undefined} styleVersions={styleVersions} activeVersionId={styleVersions.find((row) => row.styleKey === submittedStyleKey && row.designUrl === finalDesignPreview?.url)?.id} onRestoreVersion={restoreStyleVersion} onGeneratedPreview={acceptGeneratedPreview} onReplaceSelection={applyValidatedReplacement} onRevertInvalid={restoreLastValidStyle} onExport={exportAll} onValidationChange={(ready) => { setCompositionReady(ready); }} onCompositionChange={acceptComposition} />}
...
function StylingPanel({ family, baseItem, referenceItems = [], intent, unresolved, selections, setSelection, materialId, setMaterialId, fabricColor, setFabricColor, processId, setProcessId, ready, readyHint = '', hasDraftPatternChanges, onGenerate, printSupport, processSupport: selectedProcessSupport, onNext }: any) {
  ...
  return <div className="panel-content styling-panel">
    ...
    <button className="primary full" disabled={!ready || hasDraftPatternChanges} onClick={onNext}>{processId.endsWith('.print') ? t('确认并进入印花', 'Confirm and continue') : t('确认并导出', 'Confirm and export')}</button>
  </div>;
}
...
function MeasureCanvas({ saved }: { saved: boolean; measurements: Measurements; sex: Sex }) {
  const { t } = useLanguage();
  return <div className="measurement-workspace"><div className="measurement-guide-visual"><div className={saved ? 'measure-status saved' : 'measure-status'}>{saved ? t('✓ 尺寸已保存', '✓ Measurements saved') : t('按图示位置测量', 'Measure at the marked points')}</div><img src="/measurement/measurement-diagram.png" alt={t('人体尺寸测量位置示意', 'Body measurement diagram')} /><p>{t('软尺水平贴合身体并保持自然呼吸；建议测量两次。', 'Keep the tape level and breathe normally; measure twice.')}</p></div></div>;
}
function ReferenceGrid({ items, scores: _scores, order, selected, onSelect, onConfirm, status }: { items: ReferenceItem[]; scores: Record<string, number>; order: string[]; selected: string; onSelect: (item: ReferenceItem) => void; onConfirm: (item: ReferenceItem) => void; status: 'loading' | 'ready' | 'offline' }) {
  ...
  return <div className="reference-grid all-references">{sorted.map((item, index) => {
    const swatch = `/icon/swatch-${String((index % 12) + 1).padStart(2, '0')}.svg`;
