...
const roleGroups: Record<string, string[]> = {
  front: ['front_body', 'front_left', 'front_right', 'front_yoke'],
...
function DesignOverview({ recipe, patternContext, ready, generationRevision = 0, resetToken = 0, seedPreviewUrl, inspectUrl, inspectTick = 0, placeholderUrl, allowGenerate = true, needChoicesHint = '', onNeedChoices, onGenerated, onPending }: { recipe: CompositionRecipe; patternContext: Record<string, any>; ready: boolean; generationRevision?: number; resetToken?: number; seedPreviewUrl?: string; inspectUrl?: string; inspectTick?: number; placeholderUrl?: string; allowGenerate?: boolean; needChoicesHint?: string; onNeedChoices?: () => void; onGenerated?: (url: string, input: Record<string, any>, revision: number, seq?: number) => void; onPending?: (seq: number, active: boolean) => void }) {
  const { t } = useLanguage();
  ...
  const buildInput = (prompt?: string, view: 'front' | 'side' = 'front') => ({
  ...
  const generate = async (withPrompt = false, view: 'front' | 'side' = 'front') => {
    if (!recipe.base_case_id) return;
    const revisionAtStart = generationRevision;
    const seq = Math.max(Date.now(), genSeq.current + 1);
    genSeq.current = seq;
    const input = buildInput(withPrompt ? promptDraft : undefined, view);
    lastInputRef.current = input;
    const base = aiBase();
    inflight.current += 1;
    onPending?.(seq, true);
    onGenerated?.('', input, revisionAtStart, seq);
    setError('');
    setJob({ status: 'running', progress: 0, stage: 'generating', view });
    setPromptOpen(false);
    setImageReady(false);
    try {
      ...
      let lastKept = '';
      const keep = (job: any, onCanvas: boolean) => {
        const path = job.result_urls?.at(-1);
        if (!path) return;
        const nextUrl = `${base}${path}`;
        if (nextUrl !== lastKept) {
          lastKept = nextUrl;
          onGenerated?.(nextUrl, input, revisionAtStart, seq);
        }
        if (!onCanvas || nextUrl === imageUrlRef.current) return;
        if (imageUrlRef.current) setHistory((old) => [...old.slice(-7), imageUrlRef.current]);
        imageUrlRef.current = nextUrl;
        setImageReady(false);
        setImageUrl(nextUrl);
      };
      keep(current, seq === genSeq.current);
      if (seq === genSeq.current) setJob({ ...current, view });
      let misses = 0;
      while (!['succeeded', 'failed', 'cancelled'].includes(current.status)) {
        ...
        try {
          ...
          if (!status.ok) {
            misses += 1;
            if (misses >= 8) throw new Error(t('无法读取生成进度。', 'Unable to read generation progress.'));
            continue;
          }
          misses = 0;
      ...
      keep(current, seq === genSeq.current);
      if (current.prompt && seq === genSeq.current) setPromptDraft(String(current.prompt));
    } catch (reason) { if (seq === genSeq.current) setError(reason instanceof Error ? reason.message : t('生成失败，请重试。', 'Generation failed. Please retry.')); }
    finally {
      inflight.current = Math.max(0, inflight.current - 1);
      if (seq === genSeq.current) setJob(null);
      else if (inflight.current === 0) setJob(null);
      onPending?.(seq, false);
    }
  };
  const selectPrevious = () => {
    ...
    onGenerated?.(previous, lastInputRef.current, generationRevision, ++genSeq.current);
  };
  useEffect(() => {
    setHistory([]);
    setError('');
  ...
  const requestGenerate = (withPrompt = false, view: 'front' | 'side' = 'front') => {
    if (!allowGenerate) { onNeedChoices?.(); return; }
    void generate(withPrompt, view);
  };
  const showSample = !imageUrl && !job && Boolean(placeholderUrl);
  return <>
    {(imageUrl || (showSample && sampleUrls[sampleIndex])) && <div
      className="design-preview-frame"
      style={{ cursor: zoom > 1 ? 'grab' : 'zoom-in' }}
      onWheel={(event) => {
        event.preventDefault();
        const next = Math.max(1, Math.min(4, zoomRef.current * (event.deltaY < 0 ? 1.12 : .89)));
        setZoom(next);
    ...
    >
      {imageUrl && <img src={imageUrl} alt={t('2D设计预览', '2D design preview')} className={imageReady ? 'ready' : ''} style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }} onLoad={() => setImageReady(true)} />}
    ...
    {job && <MosaicWait label={job.view === 'side' ? t('正在生成侧视图', 'Generating the side view') : job.stage === 'enhancing' ? t('草图已出，正在出高清图', 'Draft ready, generating HD') : t('正在生成效果图', 'Generating the look')} progress={job.progress || 0} />}
    {error && <p>{error}</p>}
    <div className="design-ai-actions">
      ...
      <button className="secondary" disabled={allowGenerate && !ready} onClick={() => requestGenerate(false)}>{t('重新生成', 'Regenerate')}</button>
      <button className="secondary" disabled={allowGenerate && !ready} onClick={() => requestGenerate(false, 'side')}>{t('侧视图', 'Side view')}</button>
      <button className={`secondary ${promptOpen ? 'active' : ''}`} onClick={() => { void togglePrompt(); }}>{t('提示词', 'Prompt')}</button>
    </div>
...
export function PatternPreview({ recipe, baseCoverUrl, generationRevision = 0, resetToken = 0, seedPreviewUrl, styleVersions = [], activeVersionId = '', allowGenerate = true, needChoicesHint = '', onNeedChoices, onReset, onRestoreVersion, onGeneratedPreview, onReplaceSelection, onUndo, canUndo = false, onExport, onValidationChange, onCompositionChange, onViewMode }: {
  ...
  onGeneratedPreview?: (url: string, input: Record<string, any>, revision: number, seq?: number) => void;
  ...
  onViewMode?: (mode: 'design' | 'dxf') => void;
}) {
  const { t } = useLanguage();
  ...
  const [pendingGens, setPendingGens] = useState<number[]>([]);
  const viewModeRef = useRef<'design' | 'dxf'>('design');
  viewModeRef.current = viewMode;
  useEffect(() => { onViewMode?.(viewMode); }, [viewMode]);
  ...
  return <div className="pattern-workbench" style={{ '--fabric-color': recipe.fabric_color?.startsWith('#') ? recipe.fabric_color : '#ffffff' } as React.CSSProperties}>
    <div className={`pattern-stage ${viewMode === 'design' ? 'design-mode' : ''}`}>
      ...
      <div className="design-preview-stage" style={{ display: viewMode === 'design' ? undefined : 'none' }}>
        <DesignOverview recipe={recipe} patternContext={designPatternContext} ready={designReady} generationRevision={generationRevision} resetToken={resetToken} seedPreviewUrl={seedPreviewUrl} inspectUrl={inspect.url} inspectTick={inspect.tick} placeholderUrl={baseCoverUrl} allowGenerate={allowGenerate} needChoicesHint={needChoicesHint} onNeedChoices={onNeedChoices} onPending={(seq, active) => setPendingGens((old) => active ? (old.includes(seq) ? old : [...old, seq]) : old.filter((id) => id !== seq))} onGenerated={(url, input, revision, seq) => { onGeneratedPreview?.(url, input, revision, seq); if (url && viewModeRef.current !== 'design') setDesignPulse(true); }} />
      </div>
      {viewMode === 'design' && (styleVersions.length > 0 || seedPreviewUrl || pendingGens.length > 0) && <div className="design-version-rail" aria-label={t('搭配版本', 'Style versions')} aria-busy={pendingGens.length ? true : undefined}>
        ...
        {pendingGens.map((seq) => (
          <button key={seq} type="button" className="generating" disabled aria-busy="true" data-tip={t('正在生成试穿图', 'Generating try-on')}>
